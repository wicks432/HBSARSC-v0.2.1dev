...
iflagCenter  = 0    # 1 for int f = 0 and 0 for f(0) = 0
...
Getf = function(x,fpars){
  ...
  # Montone concave
  #----------------------------------------------------
  f = fpars$b1 *(x) - fpars$b2*exp(-fpars$b3*x) +
    fpars$b4*(x)^fpars$b5 +
    fpars$b6*log(x+fpars$b7)
  
  
    
  return(f)
}
b10 = 3   # Slope linear term
b20 = 4   # Slope -exp(b3x)
b30 = 1   
b40 = 1   # Slope x^b6
b50 = .1  # exponent
b60 = 2   # Slope log(x+b7)
b70 = 1   #
fpars    = list(b1=b10,b2=b20,b3=b30,b4=b40,b5=b50,b6=b60,b7=b70)
...
for(j in 1:ngroups){
  ...
  b1 = rnorm(1,b10,.5)   # linear slope
  b2 = rnorm(1,b20,.5)   # slope -exp(-x)
  b3 = rnorm(1,b30,.1)
  b4 = runif(1,.5*b40,2*b40)  # slope x^b5
  b5 = runif(1,.4*b50,1.2*b50)
  b6 = rnorm(1,b60,.5)        # slop log(x+x7)
  b7 = runif(1,.2*b70,2*b70)
  
  
  fpars = list(b1=b1,b2=b2,b3=b3,b4=b4,b5=b5,b6=b60,b7=b70)
