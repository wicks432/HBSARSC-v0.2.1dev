...
# ******************************************************************************
# HBSAR_Poly
#   Use orthognal polynomial instead of consines
#   HB Spectral Analysis Regression with Shape Constraints
#   DATA INPUT
...
#         All models:  
#            theta_j,k ~ N(theta_0,k,tau_k^2*exp(-k*gamma_j))       for k >  iflagZcnst
...
#          theta_{0,k} ~ N(0,eta^2(1+k/zeta_gamma)^{-eta_gamma})
#
...
# ******************************************************************************
# ******************************************************************************
"HBSAR_Poly" <- function(ydata,xdata,vdata=NULL,wdata=NULL,zdata=NULL,id_group=NULL,
                         mcmc=list(), nbasis=20, nint=100, xmin=NULL, xmax=NULL,
                         iflagPoly    = 2, # 1 = Chebychev, 2 = Jacobi, 3 = Legrendre
  ...
  # iflagPoly gives the polynomial basis
  #    1   = Chebychev
  #    2   = Jacobi
  #    3   = Legrendre
  iflagPoly = iflagPoly
  ...
  xmin      = ifelse(is.null(xmin), min(xdata), xmin)     # Minimum of xgrid
  xmax      = ifelse(is.null(xmax), max(xdata), xmax)     # Maximum of xgrid
  xrange    = xmax-xmin
  xmid      = (xmin+xmax)/2
  ...
  # Chebychev
  ChebyPoly = chebyshev(1:nbasis,kind="t")
  # Jacobi
  JacobiPoly = jacobi(1:nbasis)
  # Legrendre 
  LegPoly = legendre(1:nbasis)
  
  
  zgrid = -1 + 2*(xgrid-xmin)/xrange  # Chebychev on -1 to 1
  if (iflagPoly == 1) {
    # Chebychev Polynomials
    phixgrid <- (as.function(ChebyPoly)(zgrid))
  } 
  if (iflagPoly == 2) {
    # Jacobi Polynomials
    phixgrid <- (as.function(JacobiPoly)(zgrid))
  } 
  if (iflagPoly == 3) {
    # Legendre Polynomials
    phixgrid <- (as.function(LegPoly)(zgrid))
  } 
  phixgrid  = sqrt(2/xrange)*phixgrid
  
  # Does Z include the constant?
  if(iflagZcnst == 1){
    # Add constant to phixgrid
    phixgrid  = cbind(matrix(1,nint+1,1),phixgrid)
    kall      = c(0,kall)
    ntheta    = nbasis + 1
  }
  # ****************************************************************************
  # If free f, compute phi at xdata
  phixdata  = 0
  phi2      = 0
  # Compute phi at data if unrestricted
  if(iflagsc == 0){
    zd = -1 + 2*xdata/xrange
    if(iflagPoly == 1) phixdata <- (as.function(ChebyPoly)(zd))
    if(iflagPoly == 2) phixdata <- (as.function(JacobiPoly)(zd))
    if(iflagPoly == 3) phixdata <- (as.function(LegPoly)(zd))
    phixdata  = sqrt(2/xrange)*phixdata
    # Compute phix'phix in each group
    phi2    = array(0,dim=c(nbasis,nbasis,ngroups))
    for(j in 1:ngroups){
      phixj     = phixdata[iptHB[[j]],]
      phi2[,,j] = crossprod(phixj)
    } # End loop over groups
  } else { # End compute phi at xdata for free f
    phixdata = matrix(0,1,1)
    phi2 = array(0,dim=c(1,1,1))
  } 
  
  
  theta_names = paste("freq_",kall,sep="")
  ...
  gamma_mugg   = array(0, c(smcmc, 1)) # Smoothing parameter for upper level model
  gammallg     = array(0, c(smcmc, ngroups)) # Smoothing parameters for lower level model
  gamma_alphag = array(0, c(smcmc, 1))
  gamma_betag  = array(0, c(smcmc, 1))
  ...
  # MCMC Loop ----
  start_time = Sys.time()
  mcmc_out = get_mcmc_HBSAR(data_all=data_all, 
                            dpara=dpara,
                            xgrid=xgrid,
                            xinx=xinx-1, 
                            xover=xover, 
  ...
  met_list$gamma_met  = get_mat_to_list_met(mcmc_out$metg$gamma_met)
  ...
  met_list$gamma_met$pmet  = met_list$gamma_met$pmet/smcmc
  for (j in 1:ngroups) {
