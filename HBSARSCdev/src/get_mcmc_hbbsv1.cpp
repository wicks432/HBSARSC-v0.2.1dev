...
// [[Rcpp::export]]
Rcpp::List get_mcmc_HBBS(Rcpp::List& data_all,
                         arma::colvec& dpara,
                         arma::colvec& xgrid,
                         arma::umat& xinx,
                         arma::vec& xover,
  ...
  // theta
  mat vbn, vbn12, phixj, phi2j, thetaj, thv, thvi, bmat;
  double sse_new, sse_old, gpar, nz, nz2, t0resid2_new, t0resid2_old, testp, thvk;
  uvec zc, zvi, zv, z, zi;
  ...
  Rcpp::List fpars;
  colvec resid_new, resid_old, fxgrid_new, fxj, theta0k, fxdata_new, fxdata_star_new,
  fxdata_old, fxdata_star_old, theta_old, thetak_old, ths, met_var_new, met_var,
  met_std, hfunj_new;
  double xiparj_old=0;
  double xiparj_new=0;
  ...
  for (unsigned int imcmc=0; imcmc<nmcmcall; imcmc++) {
    ...
    for (unsigned int k = 0; k < ntheta; k++) {
      ...
      thvk = tau2all(k);  // Var(theta_jk) for j = 1, ..., J
      
      if (iflagZcnst == 1 && !bflagZ) {
        // Got intercept, which is positive with theta_00 = exp(xi)
        xi_vn       = 1/(sum(ingroup/thvk) + 1/eta02); // variance
        if (k == 0) {
    ...
    tauall = sqrt(tau2all);
    //--------------------------------------------------------------------
    // Generate eta0^2 where theta_0k ~ N(0,eta0^2)
    resid2_eta = theta0;
    if (iflagZcnst == 1) resid2_eta(0) = xipar0; // constant theta0 = exp(xipar0)
    
    sse        = accu(square(resid2_eta));
