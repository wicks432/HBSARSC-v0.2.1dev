...
// [[Rcpp::export]]
Rcpp::List get_mcmc_2DBSAR_Free(arma::colvec& ydata,
                                arma::mat& vdata,
                                arma::mat& vtv,
                                arma::mat& ptp,
  ...
  mat gammag = zeros<mat>(smcmc);    // Smoothing parameters for lower level model
  ...
  for (unsigned int imcmc=0; imcmc<nmcmcall; imcmc++) {
    ...
    // Generate Smoothing Parameter tau2 and gamma
    newsmooth = rcpp_getsmooth(tau2,gampar,egamkall,theta2,wk,tau2_s0,tau2_rn,freq_sum);
    tau2      = newsmooth(0);
    tau       = newsmooth(1);
    gampar    = newsmooth(2);
